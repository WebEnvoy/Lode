...
能力资产还必须声明它可能执行的动作类别和目标范围。Lode site capability 只声明
`read`、`prepare`、`commit` 或 `destructive`；`environment` 由 Harbor operation
catalog 声明。用户的全局策略、task/operation scope 覆盖和单次授权由 Core 拥有。
Risk hint、fixture、post-check 和 evidence requirement 不能替代用户授权，也不要求
App 默认展示运行记录。

...
[ADR 0007](0007-capability-action-declaration.md) 已解决 PD-0003：Lode site capability
声明 `read`、`prepare`、`commit`、`destructive`；Harbor operation catalog 声明
`environment`。`operation_mode` 只描述产品形态，不再承担 action risk 或授权决策。
...
|---|---|---|---|---|---|
| validate-only | 只声明待写内容、目标页面/对象、动作类别、所需资源和验证结果；只有不上传、不自动保存、不向外部传输时才可声明 `prepare`。 | 未获 `commit` 授权时不上传、不点击 submit、publish、send、delete、follow、pay 等外部变化动作。 | Core unified authorization、Harbor target binding、write post-check 和 result contract 稳定后。 | #21/#22、ADR 0002/0003/0007、research task-execution-and-admission | deferred |
...
当前阶段无 Milestone / FR / Work Item blocker。PD-0003 已解决；其余 PD-0004 至
PD-0013 为 implementation-time 或 external-contract 决策，不阻塞 #6 / #9 的第一阶段边界收敛。
...
- 当前状态: resolved by [ADR 0007](0007-capability-action-declaration.md)
- 后续归属/下一步: Lode capability 动作类别固定为 `read`、`prepare`、`commit`、`destructive`；`environment` 由 Harbor operation catalog 声明。具体 package/schema 落地由 Lode #281 的实现 PR 完成，docs-only ADR 不关闭该实现项。
